...
public class NetOverviewForm {
    ...
    private void setNetworkOverviewText(NetworkOverview networkOverview) {
        ...
        sb.append("\nRproxifier运行状态：").append(networkOverview.isNetwork_state() ? "已运行" : "已停止").append("\n\n");
        List<NetworkInterface> networkInterfaceList = networkOverview.getInterface_list();
        for (int i = 0; i < networkInterfaceList.size(); i++) {
            NetworkInterface networkInterface = networkInterfaceList.get(i);
            sb.append(String.format("网卡：%-40s\n地址：%-20s\n绑定：%s\n", networkInterface.getInterface_name(), networkInterface.getIp_addr(), isBindInterface(bindNetworkInterface, networkInterface) ? "[已绑定 OK ]" : "[未绑定]"));
            sb.append("\n");
        }
...
//        }
    ...
    private boolean isBindInterface(NetworkInterface bindNetworkInterface, NetworkInterface networkInterface) {
        return StringUtils.equals(networkInterface.getInterface_name(), bindNetworkInterface.getInterface_name());
    }
